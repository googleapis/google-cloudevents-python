...
class GraphqlError(proto.Message):
    ...
    Attributes:
        ...
        locations (MutableSequence[google.events.firebase.dataconnect_v1.types.SourceLocation]):
            ...
            ``ExecuteGraphqlRead``, ``IntrospectGraphql``,
            ``ImpersonateQuery`` and ``ImpersonateMutation``,
            ``UpdateSchema`` and ``UpdateConnector``) to reference the
            provided GraphQL GQL document.
